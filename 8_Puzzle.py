...
#returns heuristic value using misplaced tiles from the goal state
def misplaced_tiles(state):
  goal_state = [[1,2,3],[4,5,6],[7,8,0]]
  misplaced_tiles_count = 0
  for i in range(0,len(state)):
...
#returns heuristic value using manhattan distance from the goal state
#manhattan distance calculated using (x1, y1) and (x2, y2) is |x1 - x2| + |y1 - y2|
def manhattan_distance(curr_state):
   goal_state = [[1,2,3],[4,5,6],[7,8,0]]
   man_distance = 0
   pos_in_goal = dict() #stores the positions of tiles in the goal state 
...
#generats the child nodes of curr_node by moving the blank/zero LEFT, RIGHT, UP, DOWN
def expand_nodes(curr_node, visited):
  r, c = locateZero(curr_node.state)
  child_list = [] #stores the children of the curr_node 
...
def general_search(state, alg_choice):
  ...
  initial_node = Node(state)
  initial_node.heuristic_cost = heuristic
  initial_node.fsore = heuristic
 
  dq = [initial_node] #initializing the queue with the initial_node
  visited = [initial_node.state] #keeps track of the visited nodes
  ...
  while True:
    ...
    #if alg_choice != 1:
    #sort the queue in the ascending order of the
    #dq = sorted(dq, key=lambda z: (z.depth+z.heuristic_cost, z.depth))

    #sort the queue in the ascending order of the the fscore
    dq = sorted(dq, key=lambda z: (z.fscore, z.depth))
    curr_node = dq.pop(0) #pop the cheapest estimated cost node
    queue_size -= 1 
    ...
    #expands the  children of the popped node and returns the updated node with it's children 
    expanded_curr = expand_nodes(curr_node, visited)
...
def general_search(state, alg_choice):
  ...
  heuristic = 0 #default value
  #updates the heuristic cost of the node, h(n) in UCS = 0, which is set as a default value
  if alg_choice == 2:
    heuristic = misplaced_tiles(state)
